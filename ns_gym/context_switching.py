...
def make_env_with_context(env_name,context_value, context_parameter_name="masscart", seed=None):
    ...
    The context (e.g., masscart) is set to context_value and remains fixed.
    Args:
    ...
    if env_name in ["CartPole-v1","Acrobot-v1","MountainCarContinuous-v0","MountainCar-v0","Pendulum-v1"]:
        ...
        update_fn = StepWiseUpdate(scheduler,[context_value]) # Provides the context value
        ...
        scheduler = ContinuousScheduler(start=0, end=0)
        update_fn = DistributionStepWiseUpdate(scheduler,[context_value]) # Provides the context value
        ...
        if env_name == "FrozenLake-v1":
            ns_env = NSFrozenLakeWrapper(env,
                                         ...
                                         change_notification=True,
                                         delta_change_notification=True,
                                         initial_prob_dist=[context_value,(1-context_value)/2,(1-context_value)/2])
        elif env_name == "CliffWalking-v0":
            ns_env = NSCliffWalkingWrapper(env,
                                           tunable_params,
                                           change_notification=True,
                                           delta_change_notification=True,
                                           initial_prob_dist=[context_value,(1-context_value)/3,(1-context_value)/3,(1-context_value)/3])
        elif env_name == "ns_gym/Bridge-v0":
            ns_env = NSBridgeWrapper(env,tunable_params,change_notification=True,delta_change_notification=True,initial_prob_dist=[context_value,(1-context_value)/2,(1-context_value)/2])
...
def eval_target_contexts(policy, make_env_func_partial, num_episodes_per_context, target_context_range):
    """
    Evaluates a given policy across a range of target contexts.
    Args:
        policy: The trained agent/policy to evaluate.
    ...
    for i, target_ctx_val in enumerate(target_context_range):
        eval_env = make_env_func_partial(context_value=target_ctx_val) # Pass only context_value
        mean_rewards[i], std_rewards[i] = run_context_episode(policy, eval_env, num_episodes_per_context)
        eval_env.close()
...
def calculate_generalized_performance(
    ...
    for i, agent in enumerate(agent_list):
        ...
        print(f"  Evaluating Agent {i+1}/{num_agents} (trained on source context: {source_ctx:.2f})...")
    ...
    # Calculate metrics from raw U_matrix
    ...
    if normalize:
        ...
        normalization_params = {'min_reward_observed': min_r, 'max_reward_observed': max_r}
        ...
        print(f"Normalization complete. Min observed: {min_r:.2f}, Max observed: {max_r:.2f}")

    print("--- Evaluation Complete ---")
    return (U_matrix_raw, peak_perf_per_policy_raw, overall_system_perf_paper_raw, sem_overall_system_perf_paper_raw,
...
def plot_performance_curves(U_matrix_to_plot, eval_context_range, train_source_contexts, 
    ...
    for i in range(U_matrix_to_plot.shape[0]):
        plt.plot(eval_context_range, U_matrix_to_plot[i, :], label=f"Trained on Ctx: {train_source_contexts[i]:.2f}", alpha=0.7)
    
    U_envelope_to_plot = np.max(U_matrix_to_plot, axis=0)
    plt.plot(eval_context_range, U_envelope_to_plot, color='k', linestyle='--', linewidth=2, 
...
def save_metrics_to_file(filename, 
                         U_matrix_raw, peak_performances_raw, overall_system_performance_raw, sem_overall_raw,
                         U_matrix_normalized, peak_performances_normalized, overall_system_performance_normalized, sem_overall_norm,
                         normalization_params,
                         source_contexts, context_range, args_dict):
    """
    ...
    try:
        with open(filename, 'w') as f:
            ...
            f.write(f"Mean: {overall_system_performance_raw:.4f}, SEM: {sem_overall_raw:.4f}\n")
            f.write("\nPeak Performance for each Policy (Raw):\n")
            for i, peak_p in enumerate(peak_performances_raw):
                f.write(f"  Agent trained on Ctx {source_contexts[i]:.2f}: Max Reward = {peak_p:.4f}\n")
            ...
            # --- Normalized Metrics ---
            if U_matrix_normalized is not None and overall_system_performance_normalized is not None and sem_overall_norm is not None:
                f.write("\n\n--- NORMALIZED METRICS ---\n")
                f.write(f"Normalization Parameters: Min Observed = {normalization_params.get('min_reward_observed', 'N/A'):.4f}, Max Observed = {normalization_params.get('max_reward_observed', 'N/A'):.4f}\n")
                f.write("Overall System Generalized Performance (Normalized):\n")
                f.write(f"Mean: {overall_system_performance_normalized:.4f}, SEM: {sem_overall_norm:.4f}\n")
                ...
                for i in range(U_matrix_normalized.shape[0]):
                    row_str = ", ".join([f"{val:.4f}" for val in U_matrix_normalized[i, :]])
                    f.write(f"Agent {i} (Ctx {source_contexts[i]:.2f}): [{row_str}]\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--context_param", type=str, default="masscart", help="Environment parameter to modify (e.g., 'masscart', 'length').")
    parser.add_argument("--num_target_contexts", type=int, default=100, help="Number of points in the target context range for evaluation.")
    parser.add_argument("--target_context_min", type=float, default=0.1, help="Minimum value for the target context range.")
    ...
    parser.add_argument("--output_dir", type=str, default=default_output_dir, help="Directory to save plot and metrics files.")
    parser.add_argument("--plot_file", type=str, default="performance_plot.png", help="Filename for the saved plot (relative to output_dir).")
    parser.add_argument("--metrics_file", type=str, default="performance_metrics.txt", help="Filename for the saved metrics (relative to output_dir).")
    parser.add_argument("--normalize_rewards",type=bool,default=True,help="normalize generalized performance" )
    ...
    current_context_range = np.linspace(args.target_context_min, args.target_context_max, args.num_target_contexts)
    ...
    make_env_partial_fn = partial(make_env_with_context, env_name=args.env_name,context_parameter_name=args.context_param)

    print(f"\n--- Training {len(SOURCE_CONTEXTS)} Agents ---")
    for i, s_ctx in enumerate(SOURCE_CONTEXTS):
        print(f"Training Agent {i+1}/{len(SOURCE_CONTEXTS)} for source context ({args.context_param}): {s_ctx:.2f}")
    ...
    if overall_norm is not None and sem_norm is not None:
        print(f"Overall System Generalized Performance (Paper's V-metric, Normalized): Mean = {overall_norm:.2f}, SEM = {sem_norm:.4f}")



    print("\nSaving metrics and plotting performance curves...")
    save_metrics_to_file(metrics_filepath, 
                         U_raw, peaks_raw, overall_raw, sem_raw,
                         U_norm, peaks_norm, overall_norm, sem_norm, norm_params,
                         SOURCE_CONTEXTS, current_context_range, vars(args))
    
    # Decide which matrix to plot based on normalization flag
    if args.normalize_rewards and U_norm is not None:
        plot_performance_curves(U_norm, current_context_range, SOURCE_CONTEXTS, overall_norm, args.context_param, True, plot_filepath)
    else:
        plot_performance_curves(U_raw, current_context_range, SOURCE_CONTEXTS, overall_raw, args.context_param, False, plot_filepath)
