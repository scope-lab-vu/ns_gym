...
In the CliffWalking environment we directly modify the "slipperiness" of the gridworld, which affects the probability of the agent's intended action being altered to a different action. By default, the environment is fully deterministic (i.e., no slip). As the slipperiness increases the probability of moving in the intended direction decreases, while the probability of moving in the reverse or perpendicular directions increases by an equal amount. For example, the initial categorical distribution over possible next states for action "right" is [1.0,0.0,0.0,0.0] (i.e., 100% chance of moving right). If we decrease the chance of going in the intended direction by 0.4, NS-Gym updates categorical distribution as [0.6,0.1,0.1,0.1] (i.e., 60% chance of moving right, 10% chance of moving left, 10% chance of moving up, and 10% chance of moving down).


| Parameter | Description | Default Value |
|-----------|-------------|---------------|
| `P` | Categorical distribution over next states for each action | Deterministic (no slip) [1.0, 0.0, 0.0, 0.0] |
