...
MCTS does know the the ground truth transition function.
"""

current_datetime = datetime.datetime.now()
formatted_date = current_datetime.strftime('%Y-%m-%d_%H:%M:%S')
experiment_name = f"CartpoleVanillaMCTS_withChangNotif_{formatted_date}"
...
def run_single_experiment(mass, gamma, c, num_iter, sample_id,seed):
    ...
    """
    realworld_env = make_env(mass=mass, change_notification = True, delta_change_notification = False, in_sim_change = False)
...
def run_all_experiments(mass, gamma, c, num_iter, sample_id,q,seed):
    ...
    try:
        logger.info(f"STARTED: Carpole Vanilla MCTS with notit. experiment with pole mass: {mass}, gamma: {gamma}, c: {c}, num_iter: {num_iter}, num_samples: {sample_id}")
        start_time = time.time()
        episode_reward = run_single_experiment(mass, gamma, c, num_iter, sample_id,seed)
        total_time = time.time() - start_time
        result = [sample_id,episode_reward,"Vanilla MCTS with notif,negative reward.",mass,gamma,c,num_iter,total_time,seed]
        logger.info(f"FINISHED: Carpole Vanilla MCTS with change notit. experiment with pole mass: {mass}, gamma: {gamma}, c: {c}, num_iter: {num_iter}, num_samples: {sample_id}, in {total_time} seconds.")
        q.put(result)
...
def main():
    ...
    mass = [0.1]
    gamma = [0.99,0.85, 0.75, 0.5, 0.25, 0.1]
    c = [2, np.sqrt(2), 1, 0.5, 0.1, 0.01]
    num_iter = [100]
    sample_id= [x for x in range(10)]
...
if __name__ == "__main__":
    print("Starting Exp, Cartpole Vanilla MCTS with change notit.")
    main()
