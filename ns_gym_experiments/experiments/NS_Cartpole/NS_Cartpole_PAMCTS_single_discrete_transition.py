...
def run_all_experiments(mass,num_iter,c,gamma,alpha,sample_id,q,seed,agent_config,wrapper_config,gym_config):
    ...
    try:
        ...
        result = [mass,sample_id,episode_reward,experiment_name,num_iter,total_time,seed,c,gamma,alpha]
        q.put(result)
...
def main():
    ...
    num_experiments = len(sample_id) * len(num_iter) * len(c) * len(gamma) * len(alpha) * len(masses)
    ...
    with open(outfile, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["mass","sample_id","reward","experiment_name","num_iter","total_time","seed","c","gamma","alpha"])

    writer_process = multiprocessing.Process(target=write_results_to_file, args=(queue,outfile))
    writer_process.start()

    parameter_combinations = itertools.product(masses,num_iter,c,gamma,alpha,sample_id)
