...
In this experiment we vary gravity amoung the following values: 9.8,20,50,500, The initial value of g  is 9.8. 
...
experiment_name = f"CartpoleVanillaMCTS_Changing_Gravity_withoutChangNotif_{formatted_date}"
...
def make_env(gravity,change_notification,delta_change_notification,in_sim_change):
    """Make non-stationary FrozenLake environment.
    """
    env = gym.make("CartPole-v1",max_episode_steps=2500)
    gravity_pole_scheduler = nsb.schedulers.DiscreteScheduler({0})
    gravity_pole_updateFn = nsb.update_functions.StepWiseUpdate(gravity_pole_scheduler,[gravity])
    params = {"gravity":gravity_pole_updateFn}
...
def run_single_experiment(gravity, gamma, c, num_iter, sample_id,seed):
    ...
    """
    realworld_env = make_env(gravity=gravity, change_notification = False, delta_change_notification = False, in_sim_change = False)
...
def run_all_experiments(gravity, gamma, c, num_iter, sample_id,q,seed):
    ...
    try:
        logger.info(f"STARTED: Carpole Vanilla MCTS no change notit. experiment with gravity: {gravity}, gamma: {gamma}, c: {c}, num_iter: {num_iter}, num_samples: {sample_id}")
        start_time = time.time()
        episode_reward = run_single_experiment(gravity, gamma, c, num_iter, sample_id,seed)
        total_time = time.time() - start_time
        result = [sample_id,episode_reward,"Vanilla MCTS no change notif,negative reward,changing gravity.",gravity,gamma,c,num_iter,total_time,seed]
        logger.info(f"FINISHED: Carpole Vanilla MCTS no change notit. experiment with gravity: {gravity}, gamma: {gamma}, c: {c}, num_iter: {num_iter}, num_samples: {sample_id}, in {total_time} seconds.")
        q.put(result)
...
def main():
    ...
    queue = manager.Queue()
    gravity = [9.8,20,50,500]
    gamma = [0.999]
    c = [50]
    num_iter = [25, 50, 75, 100, 200, 300]
    sample_id= [x for x in range(50)]
    num_experiments = len(gravity) * len(gamma) * len(c) * len(num_iter) * len(sample_id)
    ...
    with open(outfile, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["sample_id","reward","experiment_name","gravity","gamma","c","num_iter","total_episode_time","seed"])


    parameter_combinations = itertools.product(gravity, gamma, c, num_iter, sample_id)
