...
config_file_path = "/media/--/home/n--/ns_gym/experiments/NS_Cartpole/configs/CartPole_AlphaZero_continous_update.yaml"
...
def run_all_experiments(num_iter,sample_id,c,gamma,q,seed,agent_config,wrapper_config,gym_config):
...
def main():
    ...
    num_experiments = len(sample_id) * len(num_iter) 
    ...
    parameter_combinations = itertools.product(num_iter,sample_id)
    input = [(*params,c,gamma,queue,seeds[i],agent_config,wrapper_config,gym_config) for i,params in enumerate(parameter_combinations)]
