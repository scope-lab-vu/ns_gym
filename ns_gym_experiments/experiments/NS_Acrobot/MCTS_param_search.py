...
"""
MCTS Hyper parameter search for Acrobot environment
"""
...
config_file_path = "/media/--/home/n--/ns_gym/experiments/NS_Acrobot/MCTS_hyperparm_search.yaml"
...
def make_env(gym_config, wrapper_config):
    """Make Acrobot environment."""
    realworld_env = gym.make(gym_config["name"])
    return realworld_env

def run_all_experiments(num_iter, c, gamma, sample_id, q, seed, agent_config, wrapper_config, gym_config):
    """Run all experiments with the given grid search over parameters."""
    try:
        logger.info(f"STARTING: Experiment with num_iter: {num_iter}, sample_id: {sample_id}, seed: {seed}, c: {c}, gamma: {gamma}")
        ...
        while not done and not truncated:
            mcts_agent = nsb.benchmark_algorithms.MCTS(realworld_env, obs, agent_config["max_mcts_search_depth"], m=num_iter, c=c, gamma=gamma)
        ...
        logger.info(f"FINISHED: Experiment with num_iter: {num_iter}, sample_id: {sample_id}, seed: {seed}, c: {c}, gamma: {gamma}")
