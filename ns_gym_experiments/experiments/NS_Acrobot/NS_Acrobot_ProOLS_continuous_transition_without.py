...
from ns_gym.benchmark_algorithms.ProOLS_ns import ProOLSAgent, Config
...
config_file_path = 'experiments/NS_Acrobot/Acrobot_ProOLS_continuous_update.yaml'
...
def run_all_experiments(num_iter, c, gamma, alpha, sample_id, q, seed, agent_config, wrapper_config, gym_config):
    ...
    try:
        ...
        # Load the trained ProOLS model
        pro_ols_agent = ProOLSAgent(config)
        pro_ols_agent.actor.load_state_dict(torch.load(model_path))
        pro_ols_agent.actor.eval()
        episode_reward = 0
        state, _ =  realworld_env.reset(seed=random.randint(0, 100000))

        t = 0
        while True:
            action, _, _ = pro_ols_agent.get_action(state)
