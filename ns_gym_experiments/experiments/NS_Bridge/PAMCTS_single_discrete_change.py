...
config_file_path = "/media/--/home/n--/ns_gym/experiments/NS_Bridge/configs/PAMCTS_single_config.yaml"
...
def run_all_experiments(sample_id,alpha,p,q,seed,agent_config,wrapper_config,gym_config):
    ...
    try:
        ...
        result = [sample_id,episode_reward,experiment_name,num_mcts_simulations,total_time,seed,realworld_env.P,count,alpha,p]
        q.put(result)
...
def main():
    ...
    num_experiments = len(sample_id) * len(alpha) * len(p)
    ...
    with open(outfile, mode='w', newline='') as file:
        ...
        writer.writerow(["sample_id","reward","experiment_name","num_iter","total_time","seed","final_dist","num_steps","alpha","p"])

    writer_process = multiprocessing.Process(target=write_results_to_file, args=(queue,outfile,logger))
    writer_process.start()

    parameter_combinations = itertools.product(sample_id,alpha,p)
