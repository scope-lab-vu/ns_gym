...
config_file_path = "/media/--/home/n--/ns_gym/experiments/NS_CliffWalking/configs/PAMCTS_single_config.yaml"
...
def run_all_experiments(p,num_iter,c,gamma,alpha,sample_id,q,seed,agent_config,wrapper_config,gym_config):
    """Run all experiments with the given gridsearch over parameters.
    """
    try:
        logger.info(f"STARTING: Experiment with p:{p}, num_iter:{num_iter} num_samples: {sample_id},seed: {seed},c {c}, gamma {gamma},alpha {alpha}")
        ...
        result = [p,sample_id,episode_reward,experiment_name,num_iter,total_time,seed,c,gamma,alpha]
        q.put(result)
...
def main():
    ...
    num_experiments = len(sample_id) * len(num_iter) * len(c) * len(gamma) * len(alpha) * len(p)
    ...
    with open(outfile, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["p","sample_id","reward","experiment_name","num_iter","total_time","seed","c","gamma","alpha"])

    writer_process = multiprocessing.Process(target=write_results_to_file, args=(queue,outfile))
    writer_process.start()

    parameter_combinations = itertools.product(p,num_iter,c,gamma,alpha,sample_id)
