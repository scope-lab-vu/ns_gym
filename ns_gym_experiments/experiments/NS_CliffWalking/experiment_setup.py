...
def make_contin_env(wrapper_config):
    ...
    scheduler = ns_gym.schedulers.ContinuousScheduler(start=0,end=10)
    ...
    realworld_env = ns_gym.wrappers.NSCliffWalkingWrapper(env,
...
def make_discrete_env(p,wrapper_config):
    ...
    updateFn = ns_gym.update_functions.DistributionStepWiseUpdate(scheduler,update_values=[[p,(1-p)/3,(1-p)/3,(1-p)/3]])
    tunable_params = {"P":updateFn}
    env = gym.make("CliffWalking-v0",max_episode_steps=200)
    realworld_env = ns_gym.wrappers.NSCliffWalkingWrapper(env,
                                                      tunable_params=tunable_params,
