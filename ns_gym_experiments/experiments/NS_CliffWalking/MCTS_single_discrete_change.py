...
config_file_path = "/media/--/home/n--/ns_gym/experiments/NS_CliffWalking/configs/MCTS_single_change_config.yaml"
...
def run_all_experiments(p,num_iter,c,gamma,sample_id,q,seed,agent_config,wrapper_config,gym_config):
    ...
    """
    try:
        logger.info(f"STARTING: Experiment with p:{p}, num_iter:{num_iter} num_samples: {sample_id},seed: {seed},c {c}, gamma {gamma}")

        ########## Setup the environment ##########
        realworld_env = make_env(p,gym_config=gym_config,wrapper_config=wrapper_config)
        mcts_depth = agent_config["mcts_depth"]
        ...
        max_steps = gym_config["max_episode_steps"]
        count = 0
        
        while not done and not truncated and count < max_steps:
            planning_env = realworld_env.get_planning_env()
            MCTS_agent = nsb.benchmark_algorithms.MCTS(planning_env,state=obs,d=mcts_depth,m=num_iter,c=c,gamma=gamma)
        ...
        logger.info(f"FINISHED: Experiment with p: {p},num_iter:{num_iter} num_samples: {sample_id},seed: {seed},c {c}, gamma {gamma}")
        result = [p,sample_id,episode_reward,experiment_name,num_iter,total_time,seed,c,gamma]
        q.put(result)
...
def main():
    ...
    num_experiments = len(sample_id) * len(num_iter) * len(c) * len(gamma) * len(p)
    ...
    with open(outfile, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["p","sample_id","reward","experiment_name","num_iter","total_time","seed","c","gamma"])
