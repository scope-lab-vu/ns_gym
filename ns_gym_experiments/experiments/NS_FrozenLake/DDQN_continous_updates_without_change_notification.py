...
experiment_name = f"DDQN_ContinuousUpdate_withoutChangNotif_{formatted_date}"
...
# model_path_2 = script_dir  / ".." / ".." / "ns_gym" / "benchmark_algorithms" / "DDQN" / "DDQN_models" / "FrozenLake_DDQN_07.pth"
...
def run_single_experiment(sample_id,seed):
    ...
    """
    realworld_env = make_env(change_notification = False, delta_change_notification = False, in_sim_change = False)
...
def run_all_experiments(sample_id,q,seed):
    ...
    """
    try:
        logger.info(f"STARTING: Experiment with sample_id: {sample_id},seed: {seed}")
        start_time = time.time()
        episode_reward = run_single_experiment(sample_id,seed=seed)
        total_time = time.time() - start_time
        logger.info(f"FINISHED: {sample_id},seed: {seed} in {total_time} seconds.")
        result = [sample_id,episode_reward,experiment_name,total_time,seed]
        q.put(result)
...
def main():
    ...
    with open(outfile, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["sample_id","reward","experiment_name","total_time","seed"])
