...
config_file_path = "/media/--/home/n--/ns_gym2/ns_gym/experiments/NS_FrozenLake/configs/PAMCTS_continous_update_with_notification.yaml"
...
def make_env(gym_config,wrapper_config):
    ...
    env = gym.make(gym_config["name"],max_episode_steps=gym_config["max_episode_steps"],is_slippery=gym_config["is_slippery"])
    ...
    scheduler = nsb.schedulers.ContinuousScheduler(start=1,end = 3)
    updateFn = nsb.update_functions.DistributionDecrmentUpdate(scheduler,k=0.2)
    params = {param_name:updateFn}
    realworld_env = nsb.wrappers.NSFrozenLakeWrapper(env,params,change_notification,delta_change_notification,in_sim_change,initial_prob_dist=[1,0,0])
...
def run_all_experiments(num_iter,c,gamma,alpha,sample_id,q,seed,agent_config,wrapper_config,gym_config):
    ...
    try:
        ...
        max_experiment_steps = exp_config["max_experiment_steps"]        
        exp_step = 0
        ...
        while not done and not truncated and exp_step < max_experiment_steps:
            planning_env = realworld_env.get_planning_env()
            action , _ = PAMCTS_agent.act(obs,planning_env) 
