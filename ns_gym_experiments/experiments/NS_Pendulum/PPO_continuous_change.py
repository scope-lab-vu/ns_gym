...
def make_env(config):
    ...
    update_fn = ns_gym.update_functions.IncrementUpdate(scheduler,0.01)
    param_map  = {tunable_param[0]:update_fn}
    ns_env = ns_gym.wrappers.NSClassicControlWrapper(env,param_map, change_notification = config["change_notification"])
    return ns_env
